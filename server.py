...
@app.route('/add-timestamp', methods=['POST', 'GET'])
def add_timestamp():

    if request.method == 'GET':
        bucket = request.args.get("bucket")
        object_key = request.args.get("object-key")
        _find_time(bucket, object_key)
        ...
        if message["detail-type"] == "Object Created":
            app.logger.info(f"Received object created message. {message}")
...
def _find_time(bucket, object_key):
    ...
    try:
        ...
        app.logger.warning(f"Problem occurred while writing {item_to_write} to DynamoDB table {table_name}.",
                           exc_info=e)
